...
public sealed class SettingsCatalogTests
{
    ...
    [Fact]
    public void Glide_catalog_has_193_current_and_no_future_entries()
    {
        Assert.Equal(193, SettingsCatalog.All.Count);
        Assert.Equal(193, SettingsCatalog.All.Count(x => x.FuturePhase is null));
        ...
        Assert.Contains(SettingsCatalog.All, x => x.Id == "overlay.animationAvoidOverlap" && x.FuturePhase is null && x.DefaultValue is bool enabled && enabled);
        Assert.Contains(SettingsCatalog.All, x => x.Id == "overlay.animationRefreshRateHz" && x.FuturePhase is null && x.DefaultValue is int refreshRate && refreshRate == 144);
        ...
        Assert.Contains(SettingsCatalog.All, x => x.Id == "folderNav.order" && x.FuturePhase is null && Equals(x.DefaultValue, "Alphabetical"));
