...
public partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        Viewport.BrowseRequested += async (_, delta) => await NavigateAsync(delta);
        ...
        Closing += async (_, e) =>
        {
            ...
            var enterSpeedBoostStandby = _settings.SpeedBoostEnabled && GlideWindowRegistry.Snapshot().Count <= 1;
    ...
    private SlideshowSessionController EnsureSlideshow()
    {
        ...
        var slideshow = new SlideshowSessionController(
            ...
            () => WindowState == WindowState.FullScreen, ToggleFullscreen, direction => TryNavigateSiblingFolderAsync(direction, forceDirectionalEdge: true),
            direction => { if (direction < 0) _navigator.Last(); else _navigator.First(); },
            direction => _navigator.Move(direction), index => _navigator.SelectIndex(index),
            () => PresentCurrentAsync(), UpdateSlideshowUi,
    ...
    private async Task<PresentationOutcome> PresentCurrentAsync(bool rapidPreviewOnly = false)
    {
        ...
        try
        {
            var result = await _loader.LoadForegroundAsync(path,
        ...
        try
        {
            ...
            _diagnostics.Write("decode", "foreground_refined",
                new { path = firstFrame.Path, request = request.ImageRequestId, refineMs = refined.DecodeTime.TotalMilliseconds,
                    cacheHit = refined.CacheHit, preparedHit = refined.PreparedFrameHit });
        }
        catch (OperationCanceledException) { }
        catch (Exception ex)
        {
    ...
    private void ScheduleRapidNavigationSettle()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                Dispatcher.UIThread.Post(async () =>
                {
                    if (token.IsCancellationRequested || expectedPath is null ||
                        !string.Equals(_navigator.Current, expectedPath, StringComparison.OrdinalIgnoreCase)) return;
                    _loader.ReportNavigationActivity(ImageNavigationActivity.NormalBrowse);
                    await PresentCurrentAsync(rapidPreviewOnly: false);
    ...
    private async Task<bool> TryNavigateSiblingFolderAsync(int direction, bool forceFirstImage = false, bool forceDirectionalEdge = false, bool rapidPreviewOnly = false)
    {
        ...
        await PresentCurrentAsync(rapidPreviewOnly: rapidPreviewOnly);
        return true;
    }
    ...
    private void PreviousFolderTopClicked(object? sender, RoutedEventArgs e) => _ = TryNavigateSiblingFolderAsync(-1, forceFirstImage: true);
    private void NextFolderTopClicked(object? sender, RoutedEventArgs e) => _ = TryNavigateSiblingFolderAsync(1, forceFirstImage: true);
    private void PreviousFolderClicked(object? sender, RoutedEventArgs e) => _ = TryNavigateSiblingFolderAsync(-1);
    private void NextFolderClicked(object? sender, RoutedEventArgs e) => _ = TryNavigateSiblingFolderAsync(1);
    ...
    private async void PreviousClicked(object? sender, RoutedEventArgs e) => await NavigateAsync(-1);
    private async void NextClicked(object? sender, RoutedEventArgs e) => await NavigateAsync(1);
    private async void FirstClicked(object? sender, RoutedEventArgs e) { _navigator.First(); await PresentCurrentAsync(); }
    private async void LastClicked(object? sender, RoutedEventArgs e) { _navigator.Last(); await PresentCurrentAsync(); }
    ...
    private async void ReliableCommandButtonPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        e.Handled = true;
        if (ReferenceEquals(button, StatusPreviousButton)) await NavigateAsync(-1);
        else if (ReferenceEquals(button, StatusNextButton)) await NavigateAsync(1);
        else if (ReferenceEquals(button, StatusFirstButton)) { _navigator.First(); await PresentCurrentAsync(); }
        else if (ReferenceEquals(button, StatusLastButton)) { _navigator.Last(); await PresentCurrentAsync(); }
    ...
    private void ExecuteCaptionAction(string? action)
    {
        switch (action)
        {
            ...
            case "Close Glide":
                ...
                if (!TrySendNativeSystemCommand(ScClose)) Close();
                break;
    ...
    private void StopHeldNavigation(bool settle)
    {
        _heldNavigationCts?.Cancel();
    ...
    private async Task<bool> NavigateRapidStepAsync(int delta, CancellationToken token)
    {
    ...
    private Task ExecuteCommandAsync(GlideCommand command) => _commands.ExecuteAsync(command);
    ...
    private MainWindowCommandDispatcher CreateCommandDispatcher()
    {
        ...
        var actions = new Dictionary<GlideCommand, Func<Task>>
        {
            ...
            [GlideCommand.Home] = async () =>
            {
                if (string.Equals(_settings.HomePageMode, "Browser page", StringComparison.OrdinalIgnoreCase))
                {
                    var browser = _workspace.Tabs.OfType<BrowserTabState>().FirstOrDefault(t => string.Equals(t.Folder, ResolveHomeBrowserFolder(), StringComparison.OrdinalIgnoreCase)) ?? (BrowserTabState)CreateConfiguredHomeDestination();
                    _workspace.Select(browser.Id);
                }
                else
                {
                    var home = _workspace.Tabs.FirstOrDefault(t => t.Kind == TabKind.Home) ?? CreateConfiguredHomeDestination();
                    _workspace.Select(home.Id);
                }
                await ActivateWorkspaceAsync();
            },
            [GlideCommand.PreviousImage] = () => NavigateAsync(-1), [GlideCommand.NextImage] = () => NavigateAsync(1),
            [GlideCommand.FirstImage] = async () => { _navigator.First(); await PresentCurrentAsync(); }, [GlideCommand.LastImage] = async () => { _navigator.Last(); await PresentCurrentAsync(); },
            [GlideCommand.PreviousFolder] = () => TryNavigateSiblingFolderAsync(-1), [GlideCommand.NextFolder] = () => TryNavigateSiblingFolderAsync(1),
    ...
    private void ClearPresentedImageForSpeedBoostStandby()
    {
        ...
        Viewport.PresentationRequestId = 0;
