...
public partial class MainWindow : Window
{
    ...
    private async Task OpenAsImageTabAsync(string path, bool openInNewTab = false)
    {
        ...
        {
            Title = "Glide 4.3.1 — Unsupported or missing image";
            return;
        }
        ...
        catch (Exception ex)
        {
            if (IsRequestCurrent(request))
            {
                ...
                Title = string.IsNullOrEmpty(extension)
                    ? $"Glide 4.3.1 — Open failed: {ex.GetType().Name}"
                    : $"Glide 4.3.1 — No decoder for {extension} (optional codec required)";
            }
            if (_warmPresentationGateActive)
                await ReleaseWarmPresentationGateAsync(safeFrameReady: false);
    ...
    private void ShowHomeSurface()
    {
        ...
        ApplyWelcomeLayout();
        Title = recentLanding ? "Glide 4.3.1 — Recent pictures" : "Glide 4.3.1 — Home";
    ...
    private void NavigateBrowserTo(string folder, bool addHistory, string? highlightPath = null)
    {
        ...
        Title = $"Glide 4.3.1 — Explorer — {folder}";
        RebuildTabStrip();
        SelectBrowserHighlight(id);
        UpdateTabNavigationButtons();
    ...
    private void UpdateWindowTitle(string? prefix = null)
    {
        ...
        {
            if (HomeHost.IsVisible) Title = "Glide 4.3.1 — Home";
            return;
        }
        ...
        Title = prefix is null
            ? $"Glide 4.3.1 — {display}  {index}  {Viewport.ZoomPercent}%"
            : $"Glide 4.3.1 — {prefix} — {display}";
    }
