...
public sealed class WindowInWindowOverlayManager : IDisposable
{
    ...
    public Action? FullscreenNavigationRequested { get; set; }
    ...
    public async Task<bool> AddAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var result = await _loader.LoadFullForegroundAsync(path, cancellationToken).ConfigureAwait(true);
    ...
    public async Task RestoreAsync(IEnumerable<OverlayState> states, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var state in states)
        {
            ...
            try
            {
                var result = await _loader.LoadFullForegroundAsync(state.Path, linked.Token).ConfigureAwait(true);
    ...
    private void SurfacePointerPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        {
            var chromeHit = _chromeVisibleId == item.State.Id &&
    ...
    private void SurfacePointerReleased(object? sender, PointerReleasedEventArgs e)
    {
        ...
        if (fullscreenClick)
        {
            ...
            FullscreenNavigationRequested?.Invoke();
            e.Handled = true;
        }
        ScheduleChromeHide();
    ...
    private async Task RefreshSelectedAsync(Guid id)
    {
        ...
        try
        {
            ...
            var result = await _loader.LoadFullForegroundAsync(path, _lifetime.Token).ConfigureAwait(true);
