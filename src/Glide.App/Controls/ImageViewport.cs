...
public sealed class ImageViewport : Control
{
    ...
    public long PresentationRequestId { get; set; }
    ...
    public Bitmap? Bitmap
    {
        ...
        set
        {
            ...
            _presentationSourceSize = value?.Size;
    ...
    {
        var old = GetValue(BitmapProperty);
    ...
    private Size CoordinateImageSize => _presentationSourceSize ?? Bitmap?.Size ?? default;
    ...
    public override void Render(DrawingContext context)
    {
        ...
        var bitmap = _interactionBitmapActive && _interactionBitmap is not null ? _interactionBitmap : Bitmap;
        if (bitmap is null)
        {
        ...
        // Draw through one explicit image->view matrix.  Rotation and flips are view transforms only:
        // original bitmap bytes remain untouched and selection stays in original image coordinates.
        // The bitmap can be a decoder-scaled preview. Draw its actual pixels into the destination
        // computed from the full source dimensions so preview and refinement have identical fitment.
        using (context.PushTransform(BuildImageToViewMatrix(bitmap.Size, _lastImageDest)))
            context.DrawImage(bitmap, source, source);

        if (PresentationRequestId != 0)
            PresentationDrawRecorded?.Invoke(PresentationRequestId);
    }
